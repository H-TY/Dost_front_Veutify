...
const NOTDate = computed(() => {
  ...
  // ▲ 另外抓出 "尚未" 完整預約的日期選出來，做成陣列
  const HBD = Object.keys(countMap)
    .filter((key) => countMap[key] !== bookingTimeLength)
    .map((key) => {
      return key
    })
...
const halfBookingDate = async () => {
  ...
  daysDOM.forEach((el) => {
    const btn = el.querySelector(".v-btn")
    btn?.classList.remove("half-booking")
  })

  // 若都沒有尚未完整預約的日期，直接 return，不執行後續的程式碼
  if (!NOTDate.value.HBD || NOTDate.value.HBD.length === 0) return


  // 迴圈執行，當 daysDOM 與 NOTDate.value.HBD 的日期相符時，添加 .half-booking 的 class 樣式
  daysDOM.forEach((el) => {
    const date = new Date(el.dataset.vDate).setHours(0, 0, 0, 0)
    // console.log('date', el.dataset.vDate, date)

    const isTrue = NOTDate.value.HBD.some((el) => {
      // console.log(el, new Date(el).setHours(0, 0, 0, 0))
      return new Date(el).setHours(0, 0, 0, 0) === date
    })

    if (isTrue) {
      const btn = el.querySelector(".v-btn")
      btn.classList.add("half-booking")
    }
  })
}
...
const allowedSelectDate = (date) => {
  ...
  const allow = dateGetTime > nowDate.value.getTime() && !NOTDate.value.dates.includes(dateGetTime)

  // console.log('dateGetTime', dateGetTime)
  // console.log('NOTDate', NOTDate.value.dates)
...
// ● 監視 selectedTime 欄位，再次確認使用者選擇的預約日期和時段，是否有重複
watch(selectedTime, (newST) => {
  // console.log("觸發監視 ST 欄位")
  // console.log("ST 欄位", newST)

  if (chooseBDate.value == "" || newST.length == 0) return

  if (duplicateBD(chooseBDate.value, newST)) {
    alert("選擇的日期和時段已被預約，請刷新頁面後重新選擇")
  }
})

// ● 函式：確認是否重複預約
const duplicateBD = (passInBD, passInST, ABC = alreadybookingColl.value) => {
  // console.log("passInBD", passInBD)
  // console.log("passInST", passInST)
  // console.log("alreadybookingColl", alreadybookingColl.value)

  // arry.includes(參數值) 判斷陣列是否包含指定的值，回傳 true 或 false；「這個值有沒有在陣列裡？」
  // arryA.some((el)=>{arryB.includes(el)}) 若兩邊都是陣列，要判斷是否有共同元素；「陣列裡是否存在至少一個元素符合這個條件？」
  const isDuplicate = ABC.some((el) =>
    el.bookingDate === passInBD &&
    passInST.includes(el.bookingTime)
  )
...
const submit = handleSubmit(async (values) => {
  ...
  try {
    ...
    // 更新 "尚未" 完整預約時段的日期，另外添加 class 樣式 .half-booking
    halfBookingDate()
