<template>
  <!-- Logo & 導覽列 -->
  <v-navigation-drawer class="admin-navigation-drawer" width="200" expand-on-hover rail @mouseover="isExpanded = true" @mouseleave="isExpanded = false">
    <!-- ● Logo -->
    ...
    <v-col>
      <v-sheet class="d-flex justify-center mt-6">
        <v-btn v-if="user.isLogin" prepend-icon="mdi-account-arrow-right" class="bg-deep-orange-darken-2 w-50" :rounded="isExpanded == true ? '' : 'circle'" :text="isExpanded == true ? '登出' : ''" :ripple="false" variant="outlined" @click="logout" :height="isExpanded == true ? '36' : ''"></v-btn>
      </v-sheet>
    </v-col>
  </v-navigation-drawer>
